import { ClassTypes } from '../core/constants.ts';
...
export function isClass(value: any): value is Constructor {
	return isDefined(value) && !isPrimitive(value) &&
		isFunction(Object.getPrototypeOf(value));
}

export function isMarkedAsService(value: any): value is Constructor {
	return isTruthy(getMetadata(value, ClassTypes.Service));
}

export function isMarkedAsController(value: any): value is Constructor {
	return isTruthy(getMetadata(value, ClassTypes.Controller));
}
